...
// Appointment interface - UUID-based
...
const CalendarWidget = () => {
  ...
  const [doctorId, setDoctorId] = useState<string | null>(null); // UUID

  // Get doctor ID from localStorage
  useEffect(() => {
    const userId = localStorage.getItem("user_id");
    if (userId) {
      setDoctorId(userId); // Already a UUID string
    }
  }, []);
    ...
    try {
      ...
      if (response.ok) {
        ...
        const formattedAppointments = (result.data || []).map((apt: any) => ({
          ...
          time:
            apt.appointment_time || apt.start_time?.substring(0, 5) || "00:00",
  ...
  useEffect(() => {
    ...
    if (filter !== "all") {
      return setFilteredAppointments(
        filtered.filter((app) => app.method === filter)
      );
    }
    ...
    <div className="bg-white border rounded-lg shadow-sm p-4 hover:shadow-md transition-shadow">
      ...
      <div className="flex justify-between items-center mb-2">
        ...
        <span className="text-sm font-medium text-gray-700">
          {format(calendarDays[0].date, "MMM d")} -{" "}
          {format(calendarDays[6].date, "MMM d, yyyy")}
        </span>
        <button
          onClick={() => navigateCalendar(7)}
          className="p-1 rounded-md hover:bg-gray-100 text-gray-600"
      ...
      <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1">
        {loading ? (
          ...
          filteredAppointments
            .sort((a, b) => {
              const [hoursA, minutesA] = (a.time || "00:00")
                .split(":")
                .map(Number);
              const [hoursB, minutesB] = (b.time || "00:00")
                .split(":")
                .map(Number);
                ...
                <div className="flex-grow">
                  <div className="flex justify-between">
                    <p className="font-medium text-gray-800 flex items-center">
                      ...
                      <span
                        className={`ml-2 w-2 h-2 rounded-full ${getStatusIndicator(
                          appointment.status
                        )}`}
                      ></span>
                    </p>
                    <div className="flex items-center">
                      <span className="text-xs bg-gray-100 px-2 py-0.5 rounded-full text-gray-600 flex items-center">
                        {getMethodIcon(appointment.method || "in-person")}
                        <span className="ml-1">
                  ...
                  <div className="flex items-center text-sm text-gray-500">
                    <span>
                      {formatTime(appointment.time || "00:00")} -{" "}
                      {formatTime(appointment.endTime || "00:00")}
                    </span>
                    <span
                      className={`ml-2 text-xs px-2 py-0.5 rounded-full ${getAppointmentTypeColor(
