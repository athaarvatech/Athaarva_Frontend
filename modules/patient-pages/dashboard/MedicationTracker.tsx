...
const MedicationTracker = () => {
  ...
  const getTimeUntilNextDose = (nextDose: Date) => {
  ...
  const getAdherencePrediction = (medication: { adherence: number }) => {
  ...
  const detectAlertFatigue = () => {
    ...
    const medsWithReminders = medications.filter(
      (med) => med.refillReminder
    ).length;
      ...
      <CardHeader className="pb-2 flex flex-row items-center justify-between bg-[#F0F9FA] border-b">
        ...
        <div className="flex space-x-2">
          <Button
            variant="outline"
            size="sm"
            className="h-8 bg-white text-[#006D77] border-[#006D77]"
          >
          ...
          {alertFatigue !== "low" && (
            <div className="mb-4 p-3 bg-blue-50 border border-blue-100 rounded-md">
              ...
              <p className="text-xs text-blue-600 mt-1">
                Our AI detected potential alert fatigue from multiple medication
                reminders. Consider{" "}
                {alertFatigue === "high"
                  ? "reducing frequency"
                  : "consolidating"}{" "}
          ...
          {showInteractionWarning && interactionDetails && (
            <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-md">
              ...
              <div className="mt-2 flex gap-2">
                <Button
                  size="sm"
                  className="h-7 bg-amber-600 hover:bg-amber-700 text-xs"
                >
                  Learn More
                </Button>
                <Button
              ...
              {refillSoonMeds.length > 0 && (
                <div className="p-3 border border-amber-200 rounded-lg bg-amber-50">
                    ...
                    {refillSoonMeds.map((med) => (
                      <li key={med.id} className="text-sm flex justify-between">
                        ...
                        <span className="font-medium">
                          Refill by{" "}
                          {new Date(med.refillBy).toLocaleDateString()}
                        </span>
                      </li>
                    ))}
                  </ul>
                  <div className="flex gap-2 mt-3">
                    <Button
                      size="sm"
                      className="mt-2 bg-amber-600 hover:bg-amber-700 h-7"
                    >
                      Request Refills
                    </Button>
                    <Button
                ...
                >
                  ...
                  <div className="mt-2 space-y-1">
                          ...
                          <PopoverContent className="w-80">
                            <div className="space-y-2">
                              ...
                              <p className="text-xs text-gray-600">
                                Based on your past patterns, our AI predicts
                                your adherence will be
                                <span className="font-medium">
                                  {" "}
                                  {getAdherencePrediction(medication)}%
                                </span>{" "}
                                for this medication.
                              </p>
                    ...
                    <Progress
                      ...
                      className="h-2"
                      indicatorClassName={getAdherenceColor(
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-2">
                    <Link
                      href={`/patient/medications/identify/${medication.id}`}
                    >
                      <Button
                        variant="outline"
                        size="sm"
                    ...
                    <Link
                      href={`/patient/medications/ar-demo/${medication.id}`}
                    >
                      <Button
                        variant="outline"
                        size="sm"
                  ...
                  {medication.nextDose &&
                    medication.nextDose.getTime() - new Date().getTime() <
                      1000 * 60 * 60 && (
                      <Button
                  ...
                  {/* Auto-refill options */}
                  {new Date(medication.refillBy).getTime() -
                    new Date().getTime() <
                    1000 * 60 * 60 * 24 * 7 && (
                    <div className="mt-3 border-t pt-3">
                      <h4 className="text-xs font-medium">Refill Options</h4>
                      <Link
                        href={`/patient/medications/refill/${medication.id}`}
                      >
                        <Button
