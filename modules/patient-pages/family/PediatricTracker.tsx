...
interface PediatricTrackerProps {
  immunizationData: ImmunizationData;
  childData: ChildData;
  developmentalMilestones: MilestoneGroup[];
}
...
const PediatricTracker = ({
  immunizationData,
  childData,
  developmentalMilestones,
}: PediatricTrackerProps) => {
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <p className="text-gray-600">
                    {childData.age} years old • Born{" "}
                    {new Date(childData.birthDate).toLocaleDateString()}
                  </p>
                </div>
                ...
                <div className="mt-2 md:mt-0 flex flex-wrap gap-2">
                  ...
                  {childData.nextCheckup && (
                    <Badge className="bg-blue-100 text-blue-800 border-blue-200">
                      <Calendar size={12} className="mr-1" />
                      Next Checkup:{" "}
                      {new Date(childData.nextCheckup).toLocaleDateString()}
                    </Badge>
                  )}
                </div>
              </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {immunizationData.upcoming.map((vaccine, idx) => (
                        <div
                          key={idx}
                          className="flex justify-between items-center p-2 bg-amber-50 hover:bg-amber-100 rounded-md"
                        >
                          <div>
                            <p className="font-medium">{vaccine.name}</p>
                            <p className="text-xs text-gray-600">
                              Recommended at {vaccine.recommendedAge} • Due:{" "}
                              {formatDate(vaccine.nextDue)}
                            </p>
                          </div>
                          <Badge className="bg-amber-100 text-amber-800 border-amber-200">
                            Due {formatDate(vaccine.nextDue)}
                          </Badge>
                        </div>
                      ))}
                    </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {immunizationData.completed.map((vaccine, idx) => (
                          <div
                            key={idx}
                            className="flex justify-between items-center p-2 bg-gray-50 hover:bg-gray-100 rounded-md"
                          >
                            <div>
                              <p className="font-medium">{vaccine.name}</p>
                              <p className="text-xs text-gray-600">
                                {vaccine.doses}{" "}
                                {vaccine.doses === 1 ? "dose" : "doses"} • Last
                                dose: {formatDate(vaccine.lastDose)}
                              </p>
                            </div>
                        ...
                        <div className="space-y-2">
                          {immunizationData.upcoming.map((vaccine, idx) => (
                            ...
                            >
                              ...
                              <Button
                                size="sm"
                                className="h-7 bg-[#006D77] hover:bg-[#00585F]"
                              >
                                Schedule
                              </Button>
                            </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <div className="space-y-4 pl-10 relative">
                      {[
                        ...immunizationData.completed,
                        ...immunizationData.upcoming,
                      ]
                        .sort((a, b) => {
                          const dateA = a.lastDose
                            ? new Date(a.lastDose)
                            : a.nextDue
                            ? new Date(a.nextDue)
                            : new Date();
                          const dateB = b.lastDose
                            ? new Date(b.lastDose)
                            : b.nextDue
                            ? new Date(b.nextDue)
                            : new Date();
                            ...
                            >
                              ...
                              <p className="text-xs text-gray-600">
                                {vaccine.status === "completed"
                                  ...
                                  : `Due by ${formatDate(vaccine.nextDue)}`}
                              </p>
                            </div>
                          </div>
                        ))}
              ...
              <div className="space-y-6">
                <div className="p-3 bg-[#F0F9FA] rounded-lg border border-[#E8F3F4] text-sm">
                  <p className="flex items-center text-[#006D77]">
                    ...
                    Tracking developmental progress helps ensure your child is
                ...
                <div>
                  <h3 className="font-medium mb-3">
                    Current Age Milestones ({childData.age} years)
                  </h3>
                      ...
                      .map((milestone, idx) => (
                        <div key={idx}>
                          <h4 className="font-medium mb-2">
                            Expected milestones at {milestone.age}:
                          </h4>
