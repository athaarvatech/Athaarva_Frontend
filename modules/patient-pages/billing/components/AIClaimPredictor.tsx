...
const AIClaimPredictor = () => {
  ...
  const [predictResults, setPredictResults] = useState<PredictionResult | null>(
    null
  );

  // Mock prediction data
  const mockPredictions: Record<string, PredictionResult> = {
    "70450": {
      description: "CT scan of head/brain without contrast material",
      coverageLikelihood: 92,
      estimatedReimbursement: 380.5,
      outOfPocketEstimate: 75.0,
    ...
    <Card className="border-[#E8F3F4]">
      ...
      <CardContent className="p-4">
        <p className="text-sm text-gray-600 mb-4">
          Predict insurance coverage and required documentation for medical
                ...
                <div>
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <span className="font-medium text-sm">
                      {predictResults.coverageLikelihood}%
                    </span>
                  </div>
                  <Progress
                    ...
                    className="h-2"
                    indicatorClassName={`${
                      predictResults.coverageLikelihood >= 80
                        ? "bg-green-500"
                        : predictResults.coverageLikelihood >= 50
                        ? "bg-amber-500"
                        : "bg-red-500"
                    }`}
                  />
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div>
                    ...
                    <span className="font-medium ml-1">
                      ${predictResults.estimatedReimbursement}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-600">Out of Pocket:</span>
                    <span className="font-medium ml-1">
                      ${predictResults.outOfPocketEstimate}
                    </span>
                  </div>
                </div>
