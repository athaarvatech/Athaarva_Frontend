...
export interface User {
  ...
  verification_status: VerificationStatus; // Changed from is_verified
  status: UserStatus; // Changed from is_active (implicit)
...
export interface UserProfile {
  ...
  tenant_id: string; // UUID (changed from hospital_id)
  subdomain?: string; // Tenant subdomain
  created_at: string;
...
export interface DoctorProfile {
  ...
  tenant_id: string; // UUID - hospital/tenant ID
  status: UserStatus; // Changed from is_active (implicit)
  verification_status: VerificationStatus; // Changed from is_verified (implicit)
}
...
export interface MedicalHistory {
  id: string; // UUID (changed from number)
  patient_id: string; // UUID (changed from number)
  record_type: string;
  date: string;
  description: string;
}
...
class ApiService {
  ...
  async getLongRunning<T>(url: string, config?: RequestConfig): Promise<T> {
    ...
    try {
      ...
      if ((error as any)?.code === "ECONNABORTED") {
        ...
        throw new Error(
          "Request timed out even with extended timeout. Please check the server."
        );
      }
      throw error;
    }
  ...
  async completeDoctorOnboarding(
    onboardingData: DoctorOnboardingData
  ): Promise<ApiResponse<DoctorProfile>> {
    return this.post<ApiResponse<DoctorProfile>>(
      "/doctors/onboarding/",
      onboardingData
    );
  }
  ...
  async completePatientOnboarding(
    onboardingData: PatientOnboardingData
  ): Promise<ApiResponse<PatientProfile>> {
    return this.post<ApiResponse<PatientProfile>>(
      "/patients/onboarding/",
      onboardingData
    );
  }
  ...
  async getPatientMedicalHistory(): Promise<ApiResponse<MedicalHistory[]>> {
    return this.get<ApiResponse<MedicalHistory[]>>(
