...
export function calculateAccessibilityScore(
  ...
  const avgContrast = (primaryOnWhite + secondaryOnWhite + accentOnWhite + primaryOnBlack) / 4;
...
export async function uploadBrandingAsset(
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.open('POST', `${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.UPLOADS.BRANDING}?asset_type=${type}`);
    xhr.send(formData);
  });
}
