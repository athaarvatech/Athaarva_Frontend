  ...
  return (
    <div
      className={cn(
        ...
        scale && `${scale} origin-top-left`
      )}
      style={{
        background: blueprint.palette.background,
        width:
          device === "mobile" ? "200%" : device === "tablet" ? "133%" : "100%",
      }}
    >
      <HeroSection blueprint={blueprint} />
      <div className="relative z-10 -mt-8 px-4 md:px-8 max-w-5xl mx-auto">
        <StatsStrip blueprint={blueprint} />
      </div>
      <div className="px-4 md:px-8 max-w-5xl mx-auto">
        {blueprint.about && (
          <section className="py-12">
            <AboutSection blueprint={blueprint} />
          </section>
        )}
        <section className="py-10">
          {blueprint.centersOfExcellence ? (
            <CentersOfExcellence blueprint={blueprint} />
          ) : (
            <SpecialtiesGrid blueprint={blueprint} />
          )}
        </section>
        <section className="py-10">
          <DoctorsCarousel blueprint={blueprint} />
        </section>
        {blueprint.infrastructureCards && (
          <section className="py-10">
        ...
        {blueprint.services && (
          <section className="py-10">
            <ServicesSection blueprint={blueprint} />
          </section>
        )}
        {blueprint.internationalPatients && (
          <section className="py-10">
            <InternationalSection blueprint={blueprint} />
          </section>
        )}
        <section className="py-10">
        ...
        {blueprint.news && (
          <section className="py-10">
            <NewsSection blueprint={blueprint} />
          </section>
        )}
        {blueprint.faqs && (
          <section className="py-10">
            <FAQSection blueprint={blueprint} />
          </section>
        )}
        <section className="py-10">
          <FacilitiesSection blueprint={blueprint} />
        </section>
      </div>
      ...
      <div className="relative z-10 w-full max-w-5xl mx-auto px-6 py-16">
        <div className="max-w-2xl">
          ...
          <div className="flex flex-wrap gap-3">
            <button
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg font-semibold text-sm shadow-lg"
              style={{ background: "white", color: blueprint.palette.accent }}
            >
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-1 rounded-xl overflow-hidden shadow-xl bg-white">
      {blueprint.hero.stats.map((stat, idx) => (
        <div
          ...
          style={{ borderRight: idx < 3 ? "1px solid #e2e8f0" : "none" }}
        >
          <div
    ...
    <div className="grid md:grid-cols-2 gap-8 items-center">
      <div>
        <span
          ...
          style={{ color: blueprint.palette.accent }}
        >
          {blueprint.about.subtitle}
        </span>
        <h2
          className="text-2xl md:text-3xl font-bold mt-2 mb-4"
          style={{ color: blueprint.palette.text }}
        >
        ...
        <div className="grid grid-cols-2 gap-3">
          {blueprint.about.highlights.map((h) => (
            <div
              ...
              style={{ background: blueprint.palette.accentMuted }}
            >
              <div
    ...
    <div>
      <div className="text-center mb-8">
        ...
        <p className="text-slate-500 mt-2">
          Comprehensive care across all major disciplines
        </p>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {blueprint.specialties.slice(0, 6).map((sp) => {
          ...
          return (
            <div
              key={sp.name}
              className="p-4 rounded-xl hover:shadow-md transition-shadow cursor-pointer group"
  ...
  return (
    <div>
      <div className="text-center mb-8">
        ...
        <p className="text-slate-500 mt-2">
          World-class specialized care units
        </p>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {blueprint.centersOfExcellence.map((center) => {
          ...
          return (
            <div
              key={center.name}
              className="p-4 rounded-xl text-center hover:shadow-lg transition-all cursor-pointer group"
              style={{ background: blueprint.palette.surface }}
            >
              <div
                className="w-12 h-12 rounded-full mx-auto flex items-center justify-center mb-3 group-hover:scale-110 transition-transform"
                style={{ background: blueprint.palette.accentMuted }}
              >
                <Icon
                  className="w-6 h-6"
  ...
  return (
    <div>
      <div className="text-center mb-8">
        ...
        <p className="text-slate-500 mt-2">
          Meet our world-renowned medical specialists
        </p>
      </div>
        ...
        {blueprint.doctors.slice(0, 4).map((doc) => (
          ...
          >
            ...
            <div className="p-3">
              ...
              <button
                className="w-full mt-2 py-1.5 rounded-md text-xs font-medium"
                style={{
                  background: blueprint.palette.accentMuted,
    ...
    <div>
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
        {blueprint.services.map((svc) => (
          <div
            key={svc.name}
            className="p-3 rounded-lg text-center hover:shadow-md transition-shadow cursor-pointer"
            style={{ background: blueprint.palette.surface }}
          >
            <div
      ...
      <div className="flex items-start gap-4 mb-4">
        ...
        <div>
          <h2 className="text-xl font-bold mb-2">
            {blueprint.internationalPatients.title}
          </h2>
          <p className="text-white/80 text-sm">
            {blueprint.internationalPatients.description}
          </p>
        </div>
      </div>
    ...
    >
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {blueprint.testimonials.slice(0, 2).map((t) => (
          <div
            key={t.patient || t.name || t.quote.slice(0, 20)}
          ...
          >
            ...
            <p className="text-sm text-slate-600 italic mb-3 line-clamp-3">
              &quot;{t.quote}&quot;
            </p>
            <div className="flex items-center gap-2">
              <div
                ...
                style={{ background: blueprint.palette.gradient }}
              >
                {(t.patient || t.name || "P")[0]}
              </div>
              <div>
                <div className="text-sm font-medium">{t.patient || t.name}</div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <div>
            ...
            <ul className="space-y-1.5">
              {blueprint.footer.links?.slice(0, 4).map((link) => (
                <li
                  key={link.label}
          ...
          <div>
            ...
            <div className="space-y-1.5 text-xs text-slate-400">
              <div className="flex items-center gap-2">
                <Phone className="w-3 h-3" /> 1800-XXX-XXXX
              </div>
              <div className="flex items-center gap-2">
                <Mail className="w-3 h-3" /> info@hospital.com
              </div>
              <div className="flex items-center gap-2">
