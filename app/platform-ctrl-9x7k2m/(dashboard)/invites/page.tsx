...
// Re-export the existing invites page functionality
// This page is served under the secure obfuscated route
...
const buildEmailContent = (data: ComposerState, onboardingToken: string) => {
  const friendlyTemplate = formatTemplateName(data.templateSlug);
  const onboardingUrl = `${window.location.origin}/onboarding/hospital?token=${onboardingToken}`;
  ...
  const html = `
    <div style="font-family: 'Segoe UI', Arial, sans-serif; color: #0f172a;">
      ...
      <p>
        <a href="${onboardingUrl}" style="display: inline-block; background: #007C7C; color: white; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: 600;">
          Start Onboarding
        </a>
      </p>
      <p>
...
export default function SecureInvitesPage() {
  ...
  const handleSendInvite = async () => {
    ...
    try {
      ...
      // Try to send email with onboarding link
      try {
        const emailContent = buildEmailContent(composer, result.token);
  ...
  const handleCopyOnboardingLink = (token: string) => {
    const onboardingUrl = `${window.location.origin}/onboarding/hospital?token=${token}`;
    ...
    <div className="space-y-8 text-white">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <h2 className="text-3xl font-semibold">Invitations & Onboarding</h2>
          <p className="text-white/70 max-w-2xl">
            Secure invitation management with token-based onboarding links. 
            Access protected by 2FA authentication.
          </p>
        </div>
        <div className="flex gap-3">
          ...
          <Button onClick={handleSendInvite} disabled={sending} className="bg-healthcare-primary hover:bg-healthcare-primary/80">
            {sending ? 'Sending…' : (
              <>
      ...
      <div className="grid gap-6 lg:grid-cols-[3fr,2fr]">
        ...
        <Card className="bg-white/5 border-white/10">
          <CardHeader>
            ...
            <p className="text-sm text-white/70">Creates secure token-based onboarding link sent via email.</p>
          </CardHeader>
          ...
          <CardContent className="space-y-6">
            <div className="rounded-2xl bg-white/5 border border-white/10 p-4 space-y-3 text-sm text-white/80">
              ...
              <div className="flex items-center gap-3 text-xs flex-wrap">
                <Badge className="bg-emerald-500/20 text-emerald-100 border-0">Secure Token Link</Badge>
            ...
            <div className="rounded-2xl bg-white/5 border border-white/10 p-4 space-y-2 text-sm">
              <div className="flex items-center gap-2 text-white/80">
                <ShieldCheck className="h-4 w-4" /> Onboarding Checklist
              </div>
              <ul className="list-disc list-inside text-white/60 space-y-1">
                <li>Token-validated access (no public access)</li>
      ...
      <Card className="bg-white/5 border-white/10">
        <CardHeader>
          ...
          <p className="text-sm text-white/70">All invitations with secure onboarding links.</p>
        </CardHeader>
        <CardContent className="space-y-6">
