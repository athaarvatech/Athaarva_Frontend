...
export default function MedicalRecordsPage() {
  ...
  const toggleRecordSelection = (recordId: string | string[]) => {
    if (Array.isArray(recordId)) {
      // Handle select all / deselect all
      const allSelected = recordId.every((id) => selectedRecords.includes(id));
      if (allSelected) {
        setSelectedRecords(
          selectedRecords.filter((id) => !recordId.includes(id))
        );
      } else {
        setSelectedRecords([...new Set([...selectedRecords, ...recordId])]);
      }
    } else {
      // Handle single record toggle
              ...
              <div className="h-64 bg-red-50 rounded-md flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-red-600">
                    Please try again or enter the record ID manually
                  </p>
                </div>
              </div>
            )}
          ...
          <div className="space-y-4">
            ...
            <div className="bg-blue-50 p-3 rounded-md">
              <p className="text-sm text-blue-700">
                ...
                You can set an expiration date for this link and revoke access
                at any time.
              </p>
            </div>
